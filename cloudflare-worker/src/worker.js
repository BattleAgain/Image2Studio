...
async function listTasks(request, env, url, limit) { if(!adminAuthorized(request, env, url)) return json({ ok:false, error:'admin auth required' }, 401); const rs = await env.DB.prepare('SELECT * FROM tasks ORDER BY created_at DESC LIMIT ?').bind(Math.max(1, Math.min(100, limit))).all(); return json({ ok:true, tasks:(rs.results||[]).map(redactedTask) }); }
...
async function adminStats(request, env, url){ if(!adminAuthorized(request, env, url)) return json({ok:false,error:'admin auth required'},401); const rs=await env.DB.prepare("SELECT status, COUNT(*) c FROM tasks GROUP BY status").all(); return json({ok:true,stats:rs.results||[]});}
